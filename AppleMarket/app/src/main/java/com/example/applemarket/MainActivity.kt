    ...
    private fun setRecyclerView() {
        ...
        activityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == RESULT_OK) {
                ...
                val isLike = it.data?.getBooleanExtra("isLiked",false) as Boolean

                if(isLike) {
                    dataList[itemIndex].isLiked = true
                    dataList[itemIndex].itemLikeCount += 1
                }else {
                    if(dataList[itemIndex].isLiked) {
                        dataList[itemIndex].isLiked = false
                        dataList[itemIndex].itemLikeCount -= 1
                    }
                }
        ...
        // 아이템 삭제
        adapter.itemLongClick = object : MarketAdapter.ItemLongClick {
